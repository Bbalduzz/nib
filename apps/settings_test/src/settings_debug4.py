"""Settings test - no settings update, just print."""
import nib


def main(app: nib.App):
    app.icon = nib.SFSymbol("gear")
    app.title = "Settings Debug 4"
    app.width = 300
    app.height = 200

    def on_toggle_change(value):
        print(f"[DEBUG] Toggle changed to: {value}")
        # NOT updating settings, NOT updating UI - just print

    # Settings page (no Settings object at all)
    ...
    app.build(
        nib.VStack(
            controls=[
                nib.Text("Debug Test 4 - Print only"),
