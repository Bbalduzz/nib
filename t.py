...
def _assert_country_code_is_valid(code: str) -> None:
    assert len(code) == 2, (
        f"Invalid country code: {code}, code must be 2 characters long"
    )
...
def _prepare_offers_for_countries_entry(countries: set[str]) -> str:
    ...
    main_body = _GRAPHQL_OFFERS_BY_COUNTRY_QUERY.format(
        country_entries="\n".join(offer_requests)
    )
...
def _parse_entry(json: any) -> MediaEntry:
    ...
    backdrops = [
        _IMAGES_URL + bd.get("backdropUrl") for bd in content.get("backdrops", []) if bd
    ]
...
def _parse_streaming_charts(json: any) -> StreamingCharts | None:
    if (
        ...
        or not (
            streaming_chart_info := streaming_chart_info[0].get("streamingChartInfo")
        )
...
results = search("world war Z", "IT", "it", 5, True)
pprint(results[0].offers)
